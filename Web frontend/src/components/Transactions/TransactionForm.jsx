...
const TransactionForm = () => {
  ...
  const isEdit = location.state?.transaction;
...
date: transactionToEdit?.date ? new Date(transactionToEdit.date).toISOString().split('T')[0] : "",
    ...
    validationSchema,
    onSubmit: (values) => {
      if (isEdit && transactionToEdit?._id) {
        // Update existing transaction
        updateMutateAsync({ id: transactionToEdit._id, ...values })
          .then(() => {
  ...
  // Set form values when transactionToEdit changes
  useEffect(() => {
    ...
    >
      <div className="text-center">
        <h2 className="text-2xl font-semibold text-gray-800">
          {isEdit ? "Edit Transaction" : "Transaction Details"}
        </h2>
      ...
      {(isAddTranErr || isUpdateTranErr) && (
        <AlertMessage
          type="error"
          message={
            (addTransErr || updateTransErr)?.response?.data?.message ||
      ...
      <div className="flex gap-4">
        <button
          ...
          className="mt-4 bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline transition-colors duration-200 disabled:bg-blue-300"
        >
